...
public interface IAtributoProyectoRepository extends BaseRepository<Atributo_Proyecto, Key> {
    @Query(value = "SELECT * FROM atributo_proyectos p,proyectos a   where p.value_string LIKE %:filtro% OR p.nombre_atributo LIKE %:filtro%  OR p.value_fecha LIKE %:filtro% OR a.equipo LIKE %:filtro% OR a.nombre LIKE %:filtro%",nativeQuery = true)
